...
const useApi = () => {
  ...
  const request = useCallback(
    async (
      ...
      urlencoded.append("username", "admin@xokthavy.com");
      urlencoded.append("password", "xokthavy@5d1b56c");
      ...
      var MY_BASE_URL="http://127.0.0.1:8000/api/v1/login/access-token"
      // ==================================================================
      ...
      try {
        ...
        const response = await fetch(MY_BASE_URL , requestOptions);
        // const response = await fetch(BASE_URL + endpoint, { ...params });
        if (!response.ok) {
          const data = await response.json(); // Assume always json response
          throw new Error(data.error);
        }
        // const data = await response.text(); // Assume always json response
        const data = await response.json(); // Assume always json response
